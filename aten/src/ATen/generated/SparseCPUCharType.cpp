...
Tensor & SparseCPUCharType::zeros_out(Tensor & result, IntList size) const {
    ...
    THSCharTensor_zeros(result_->tensor, size_);
    result_->maybeScalar(size.size() == 0);
    return result;
}
Tensor SparseCPUCharType::zeros(IntList size) const {
