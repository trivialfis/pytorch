...
Tensor & SparseCPUDoubleType::zeros_out(Tensor & result, IntList size) const {
    ...
    THSDoubleTensor_zeros(result_->tensor, size_);
    result_->maybeScalar(size.size() == 0);
    return result;
}
Tensor SparseCPUDoubleType::zeros(IntList size) const {
