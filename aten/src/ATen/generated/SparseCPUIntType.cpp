...
Tensor & SparseCPUIntType::zeros_out(Tensor & result, IntList size) const {
    ...
    THSIntTensor_zeros(result_->tensor, size_);
    result_->maybeScalar(size.size() == 0);
    return result;
}
Tensor SparseCPUIntType::zeros(IntList size) const {
