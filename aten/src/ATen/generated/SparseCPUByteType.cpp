...
Tensor & SparseCPUByteType::zeros_out(Tensor & result, IntList size) const {
    ...
    THSByteTensor_zeros(result_->tensor, size_);
    result_->maybeScalar(size.size() == 0);
    return result;
}
Tensor SparseCPUByteType::zeros(IntList size) const {
