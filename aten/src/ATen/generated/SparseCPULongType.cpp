...
Tensor & SparseCPULongType::zeros_out(Tensor & result, IntList size) const {
    ...
    THSLongTensor_zeros(result_->tensor, size_);
    result_->maybeScalar(size.size() == 0);
    return result;
}
Tensor SparseCPULongType::zeros(IntList size) const {
