...
Tensor & SparseCPUFloatType::zeros_out(Tensor & result, IntList size) const {
    ...
    THSFloatTensor_zeros(result_->tensor, size_);
    result_->maybeScalar(size.size() == 0);
    return result;
}
Tensor SparseCPUFloatType::zeros(IntList size) const {
