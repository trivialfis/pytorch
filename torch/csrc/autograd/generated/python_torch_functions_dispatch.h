...
inline Tensor dispatch__cudnn_init_dropout_state(const Type & ty, double dropout, bool train, int64_t dropout_seed, const Type & dtype, int64_t device) {
  maybe_initialize_cuda(dtype);
  AutoNoGIL no_gil;
  AutoGPU auto_gpu(device);
  return dtype._cudnn_init_dropout_state(ty, dropout, train, dropout_seed);
}
...
inline Tensor dispatch_empty_like(const Tensor & self, int64_t device) {
...
inline Tensor dispatch_ones_like(const Tensor & self, int64_t device) {
...
inline Tensor dispatch_rand_like(const Tensor & self, int64_t device) {
...
inline Tensor dispatch_randn_like(const Tensor & self, int64_t device) {
...
inline Tensor dispatch_zeros_like(const Tensor & self, int64_t device) {
